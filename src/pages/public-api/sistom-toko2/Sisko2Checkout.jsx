...
import { clearAll } from "../../../redux/features/siskoSlice";

const Sisko2Checkout = () => {
...
const Select = ({ children = "Select", id, value, onChange, className }) => {
  return (
    <select
      ...
      value={value}
      onChange={onChange}
      className={`${className} border rounded p-2 focus:outline-none`}
    >
      {children}
    </select>
  );
