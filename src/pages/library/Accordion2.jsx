...
function Acc1() {
  ...
  const toggleAction = (index) => setExpanded(expanded === index ? null : index);
...
function Acc3() {
  const [expanded, setExpanded] = useState(null);
  const handleExpanded = (e, item) => {
    let target = e.target.nextElementSibling;
    const content = document.getElementsByClassName("autoClose");
  ...
  return (
    <div className="w-[300px] border rounded p-2">
      {data.map((item, i) => (
        <div key={i}>
          <button
            onClick={(e) => handleExpanded(e, item)}
